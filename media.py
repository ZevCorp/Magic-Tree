...
class MediaManager:
    ...
    def record_user(self, output_path, stop_event=None):
        ...
        effective_fps = frame_count / actual_duration if actual_duration > 0 else 30.0
        logging.info(f"Actual Duration: {actual_duration:.2f}s, Effective FPS: {effective_fps:.2f}")

        # --- COMPRESSION & AUDIO MERGE STEP ---
        # Convert the AVI (MJPEG, large) to MP4 (H.264, compressed)
        # This is CRITICAL for WhatsApp to accept the file and for fast sending.
        try:
            logging.info("Compressing video and merging audio for WhatsApp...")
            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) > 1000:
                cmd = [
                    'ffmpeg', 
                    '-y', 
                    '-r', str(effective_fps), # Force input framerate to match reality
                ...
                if os.path.exists(audio_path):
                     ...
                     # Shortest prevents black video if audio is longer
                     cmd.extend(['-shortest'])
                else:
                     logging.warning("Audio file missing, compressing video ONLY.")
                     cmd.extend(['-c:v', 'libx264', '-crf', '28', '-preset', 'fast'])
                
                cmd.extend(['-pix_fmt', 'yuv420p', compressed_path])
